...
description: 'A decentralized AI safety platform live on Sui Mainnet'
---
...
<Card>
  AI agents are only as powerful as they are secure, but today, most companies ship them blind, with
  no real way to stress-test them against real-world attacks. We fix that.
</Card>

## Overview

Sui Sentinel is a crowdsourced AI red teaming platform. users deploy their AI systems on our platform as "Sentinels," and a global community of red teamers, anyone from security researchers to curious users tries to break them.

What makes us different is the trust and incentive layer underneath. Every attack is verified using cryptographic attestations generated inside a Trusted Execution Environment, and rewards are processed instantly on-chain via the Sui blockchain. You can't fake a successful attack. You can't delay a payout.

For defenders, it's an always-on, real-world stress test with a public resilience score that doubles as a security audit. For attackers, it's a skill-building game where breaking things pays real money.
...
We have built a crypto-economic system for generative AI models where discovering vulnerabilities is profitable, proving robustness is verifiable, and safety testing happens continuously rather than as formality checkboxes before deployment.
