...
description: 'How attacks are evaluated and judged in the Red Sentinel protocol.'
---

The judgment system is the heart of Red Sentinel's security guarantees. It determines whether an attack successfully violated a Sentinel's constraints through a multi-stage evaluation process that combines AI reasoning with cryptographic verification.
...
Unlike traditional systems that rely on a single judge (which could be biased, inconsistent, or exploitable), Red Sentinel uses an **ensemble jury** of multiple independent AI models running in parallel.
...
The signature is created through a cryptographic process using Ed25519 elliptic curve signatures. Checkout [Signature Geminineration](reference/signature-generation) to learn more about it.

### Why This Matters

Traditional security audits are opaque. You must trust the auditor's claims without independent verification. Red Sentinel makes every judgment:
